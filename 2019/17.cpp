...
suite s = [] {
    "2019-17"_test = [] {
        ...
        auto isIntersection = [&](int row, int col) {
            return getTile(row, col) == '#' && getTile(row - 1, col) == '#' && getTile(row + 1, col) == '#' && getTile(row, col - 1) == '#' && getTile(row, col + 1) == '#';
        };
