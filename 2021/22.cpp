...
struct Cube
{
    ...
    Cube(PointT p1, PointT p2, int sign = 1)
        : p1{p1}
        , p2{p2}
        , sign{sign}
    {
    ...
    Cube GetIntersection(const Cube &o) const
    {
        Cube ret{{},{}};
        for (int i = 0; i < 3; ++i)
        {
            ret.p1[i] = std::max(p1[i], o.p1[i]);
            ret.p2[i] = std::min(p2[i], o.p2[i]);
        }
        ret.sign = -o.sign;
        return ret;
    }

    ssize_t Volume() const
    {
        ssize_t v{sign};
        for (int i = 0; i < 3; ++i)
            v *= p2[i] - p1[i];
...
public:
    ...
    Reactor(std::istream &&is)
    {
        ...
        while (getline(is, line))
        {
                    ...
                    &c.p1[0], &c.p2[0], &c.p1[1], &c.p2[1], &c.p1[2], &c.p2[2]))
            {
                for (auto &p : c.p2)
                    ++p;
                c.sign = std::string_view{cmd} == "on" ? 1 : -1;
                cmds.emplace_back(c);
            }
        }
    }
    ...
    size_t ExecuteTest(const Cube &core, size_t max_count = std::numeric_limits<size_t>::max()) const
    {
        ...
        {
            for (int x = std::max(cmd.p1[0], core.p1[0]), x_end = std::min(cmd.p2[0], core.p2[0]);
                    x < x_end; ++x)
            {
                for (int y = std::max(cmd.p1[1], core.p1[1]), y_end = std::min(cmd.p2[1], core.p2[1]);
                        y < y_end; ++y)
                {
                    for (int z = std::max(cmd.p1[2], core.p1[2]), z_end = std::min(cmd.p2[2], core.p2[2]);
                            z < z_end; ++z)
                    {
                        if (cmd.sign > 0)
                            points.insert({x,y,z});
    ...
    size_t Execute() const
    {
        ...
        for (const auto &cmd : cmds)
        {
            ...
            {
                if (c.Intersects(cmd))
                    intersections.push_back(cmd.GetIntersection(c));
            }
            cubes.insert(cubes.end(), intersections.begin(), intersections.end());
            if (cmd.sign > 0)
                cubes.push_back(cmd);
        }

        return std::accumulate(cubes.begin(), cubes.end(), 0ll, [](ssize_t v, const Cube &c) { return v += c.Volume(); });
    }
};
...
suite s = [] {
    "2021-22"_test = [] {
        ...
        Reactor test2{std::istringstream{TEST2}};
        expect(eq(2758514936282235ull, test2.Execute()));
